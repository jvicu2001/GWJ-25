...
// Custom height terrain painting uniforms
uniform int u_height_range = 150;
uniform int u_base_height = -50;
...
vec4 height_to_color(float height) {
	if (height < 4.0) {
		return vec4(15.0/17.0, 206.0/255.0, 8.0/255.0, 1.0);
	}
	else if (height < 50.0) {
		return vec4(55.0/255.0, 209.0/255.0, 42.0/255.0, 1.0);
	}
	else if (height < 70.0) {
		return vec4(176.0/255.0);
	}
	else{
		return vec4(1.0);
	}
...
void vertex() {
	...
	vec4 tint = vec4(height_to_color(h));
	v_hole = tint.a;
